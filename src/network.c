...
static void network_send_buffer (char *buffer, size_t buffer_len, sockent_t *se) /* {{{ */
{
    ...
    user_data_t *user_data)
{
  ...
  network_send_buffer (buffer, sizeof (buffer) - buffer_free, se);
...
static int network_config_node (oconfig_item_t *ci)
{
	...
	if (cb == NULL)
	{
		ERROR("write_network plugin: malloc failed.");
		return(-1);
	}
	...
	cb->send_buffer_fill = 0;
	cb->send_buffer_ptr = cb->send_buffer;
	cb->send_buffer_vl = (value_list_t) VALUE_LIST_STATIC;
